...
**Current status:** Phase 0 through Phase 3 complete (backend skeleton, wiki assembly,
watchlist + scheduler + reliability). Phase 4 (AI pipeline) not started. Frontend not started.
See [`documentations/spec.md`](documentations/spec.md) §9 for the full task breakdown.
...
- (Planned, Phase 4) Ask an AI for a real buy/hold/sell verdict with concrete price targets and
  a suggested hold period — reasoning over *exactly* the same data the wiki page shows, never
...
flowchart TB
    subgraph ext["External data sources"]
        FH["Finnhub<br/>(primary)"]
        AV["Alpha Vantage<br/>(fallback only)"]
        GM["Gemini<br/>(AI verdict + critique, Phase 4)"]
    end

    subgraph backend["FastAPI backend"]
        ORCH["provider_orchestrator<br/>fetch_with_fallback()"]
        RL["rate_limiter"]
        CB["circuit_breaker"]
    ...
    DB[("Postgres<br/>companies / price_bars / wiki_sections /<br/>watchlist / provider_call_log / job_runs")]

    CRON1["GitHub Actions cron<br/>every 15-30 min"] -->|"POST /internal/refresh"| REFRESH
    CRON2["GitHub Actions cron<br/>daily"] -.->|"POST /internal/analyze-scheduled (Phase 4)"| GM
...
**Reliability mechanics** (Phase 3, already built): every external call goes through a
fallback orchestrator (Finnhub → Alpha Vantage), a per-provider rate limiter and circuit
breaker (both computed from an append-only `provider_call_log`, so state survives a process
restart), and jittered-backoff retries for transient errors. Every job outcome — success or
failure — is recorded in `job_runs`, so nothing fails silently. See
...
and freshness timestamps that page will eventually render.
...
# ALPHA_VANTAGE_API_KEY=...  (optional -- fallback provider, get a free key at
#                             alphavantage.co/support/#api-key)
# GEMINI_API_KEY=...         (required for Phase 4 / scripts/test_gemini_prompt.py)
...
app/
  ...
  api/routers/                — health, wiki, watchlist, refresh
  ...
  providers/                 — Finnhub (primary) + Alpha Vantage (fallback) clients
tests/
